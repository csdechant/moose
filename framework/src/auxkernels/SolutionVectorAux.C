...
SolutionVectorAux::initialSetup()
{
  ...
  {
    _var_x_name = getParam<std::string>("from_variable") + "_x";
    _var_y_name = getParam<std::string>("from_variable") + "_y";
  }
...
SolutionVectorAux::computeValue()
{
  ...
  if (isNodal())
  {
    output_x = _solution_object.pointValue(_t, *_current_node, _var_x_name);
    output_y = _solution_object.pointValue(_t, *_current_node, _var_y_name);
  }
  else
  {
    output_x = _solution_object.pointValue(_t, _current_elem->vertex_average(), _var_x_name);
    output_y = _solution_object.pointValue(_t, _current_elem->vertex_average(), _var_y_name);
  }

  RealVectorValue value(output_x, output_y, 0);
  return value;
  /*
  // The value to output
  Real output;
